    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <header className="fixed left-0 right-0 top-0 z-50 border-b border-line bg-paper/90 backdrop-blur-sm">
        <div className="mx-auto flex h-14 max-w-4xl items-center justify-between px-6 sm:px-8">
          <a href="#top" className="font-mono text-xs font-semibold uppercase tracking-wider text-terracotta hover:opacity-80">
            Krupesh Joshi
          </a>
          ...
          <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-5 text-xl leading-relaxed tracking-tight text-ink sm:text-2xl">
                Senior Frontend Engineer building performance-critical, data-heavy web applications.
              </p>
              <p className="mt-6 max-w-2xl text-[1.05rem] leading-[1.75] text-muted">
                I'm a frontend engineer with five years of experience taking React and TypeScript applications from
                prototype to production, with a particular focus on performance and architecture. I've contributed to a
                legacy AngularJS migration, shipped a Progressive Web App end to end, and cut load times and bundle
                sizes through deliberate engineering decisions rather than guesswork. Outside my day job, I contribute
                to open-source Go projects and build complete, production-quality frontend systems on my own, like the
                two case studies below.
              </p>
              <div className="mt-8 flex items-center gap-8 text-sm font-medium">
                <a href="#work" className="accent-link">View my work</a>
                <a href="#contact" className="accent-link">Get in touch</a>
              </div>
            </div>
            ...
            <div className="hidden shrink-0 flex-col items-center pt-2 md:flex">
              <div className="flex h-28 w-28 items-center justify-center rounded border border-line bg-line-subtle/60">
                <span className="font-display text-3xl font-medium tracking-tight text-terracotta">KJ</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="work" className="border-b border-line py-24">
          ...
          <div className="space-y-24">
            <CaseStudy
              ...
              tags={['React', 'TypeScript', 'Web Workers', 'TanStack Virtual', 'Zustand']}
            ...
            <CaseStudy
              ...
              tags={['React', 'TypeScript', 'Canvas 2D', 'd3-force', 'Web Workers']}
        ...
        <section className="border-b border-line py-20">
          ...
          <div className="max-w-2xl space-y-4">
            <p className="text-[0.95rem] leading-[1.65] text-muted">
              <span className="mr-1.5 font-mono text-xs font-semibold text-ink">floatpane/matcha</span>
              Fixed a memory leak in a 229-star terminal email client by replacing an unbounded cache with a
              size-capped LRU cache, merged within 24 hours.
            </p>
            <p className="text-[0.95rem] leading-[1.65] text-muted">
              <span className="mr-1.5 font-mono text-xs font-semibold text-ink">agjmills/trove</span>
              Built a full-stack sortable file-listing feature across Go and JavaScript, shipped in v0.10.0.
            </p>
          </div>
        </section>
        ...
        <footer id="contact" className="pb-16 pt-24">
          <div className="max-w-2xl space-y-8">
            <p className="font-display text-2xl font-medium leading-snug tracking-tight text-ink sm:text-3xl">
              Open to senior remote frontend roles and select freelance projects.
            </p>
            <div className="flex flex-wrap items-center gap-x-8 gap-y-3 pt-2 font-mono text-xs">
              <a href="mailto:krupeshjoshi21@gmail.com" className="accent-link">Email</a>
              <a href="https://linkedin.com/in/krupeshjoshi" target="_blank" rel="noopener noreferrer" className="accent-link">LinkedIn</a>
              <a href="https://github.com/Krupesh-21" target="_blank" rel="noopener noreferrer" className="accent-link">GitHub</a>
            </div>
...
function CaseStudy({
  ...
  return (
    <article className="space-y-5">
      ...
      <h3 className="font-display text-2xl font-medium tracking-tight text-ink sm:text-3xl">{name}</h3>
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2 pt-1">
        {tags.map((tag, i) => (
          <span key={tag} className="flex items-center gap-3">
            ...
            {i < tags.length - 1 && <span className="font-mono text-xs text-muted/40">/</span>}
          </span>
        ))}
      </div>
      <p className="max-w-2xl pt-2 text-[0.98rem] leading-[1.72] text-muted">{detail}</p>
      <div className="flex items-center gap-6 pt-3 text-sm font-medium">
        <a href={demoHref} className="accent-link">Live demo</a>
        <a href={codeHref} className="accent-link">View code</a>
      </div>
    </article>
  );
}
    ...
    <article className="space-y-2.5">
      <div className="flex flex-col justify-between gap-1 sm:flex-row sm:items-baseline">
        <h3 className="font-display text-xl font-medium text-ink">
          ...
          {note && <span className="text-base font-normal text-muted">{note}</span>}
        </h3>
        <span className="shrink-0 font-mono text-xs text-muted sm:text-right">{dates}</span>
      </div>
      <p className="max-w-2xl text-[0.95rem] leading-[1.68] text-muted">{description}</p>
    </article>
  );
}
